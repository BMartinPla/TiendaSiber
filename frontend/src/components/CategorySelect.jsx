          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((c) => (
              <button key={c.id} type="button"
                ...
                className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${String(c.id) === String(value) ? 'bg-accent-50 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300' : 'text-gray-700 dark:text-gray-200'}`}>
                {c.name}
              </button>
            ))
          )}
