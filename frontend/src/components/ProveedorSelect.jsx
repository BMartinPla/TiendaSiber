          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((p) => (
              <button key={String(p)} type="button"
                ...
                className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${String(p).toLowerCase() === q ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-200'}`}>
                {String(p)}
              </button>
            ))
          )}
